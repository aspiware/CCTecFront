...
export class FiberJobPricesComponent implements OnInit, OnDestroy {
  ...
  private loadEquipments(): void {
    ...
    this.todayService.getEquipmentsByCategory(userId, 3).subscribe({
      next: (res: any) => {
  ...
  private saveChanges(): void {
    ...
    forkJoin({
      ...
      error: async (error) => {
        console.log('[FiberJobPrices] saveChanges error', error);
        this.isSaveLoading = false;
        this.cdr.detectChanges();
        await alert({
          title: 'Error',
  ...
  private loadJobTypes(): void {
    ...
    this.todayService.getJobPricesByUser(userId, 3).subscribe({
      ...
      error: (error) => {
        console.log('[FiberJobPrices] getJobPricesByUser error', error);
        this.jobTypes = [];
        this.isLoading = false;
        this.cdr.detectChanges();
      },
  ...
  private async showSavedPopupAnchored(): Promise<void> {
    ...
    try {
      const anchorView = this.page.getViewById('fiber-main-menu-btn') as any;
      ...
      console.log('[FiberJobPrices] showSavedPopupAnchored error', error);
      await alert({
        title: 'Saved',
        message: 'Prices were saved successfully.',
        okButtonText: 'OK',
